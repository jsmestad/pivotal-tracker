  ...
  describe ".connection" do
      ...
      describe "after setting a new token" do
        ...
        it "called to RestClient::Resource using the new token" do
          RestClient::Resource.should_receive(:new).
              with(PivotalTracker::Client.api_url, headers: { 'X-TrackerToken' => "anewtoken", 'Content-Type' => 'application/xml' }, timeout: 30)
    ...
    context "with an existing token" do
      ...
      it "returned a RestClient::Resource using the token" do
        ...
        RestClient::Resource.should_receive(:new).
            with(PivotalTracker::Client.api_url, headers: { 'X-TrackerToken' => "abc123", 'Content-Type' => 'application/xml' }, timeout: 30).
            and_return(@resource)
